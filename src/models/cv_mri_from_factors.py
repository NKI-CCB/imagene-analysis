...
def cv_all_models(sfa, mri):
    ...
    for feature in mri['cad_feature']:
        for model_id, model in models.items():
            logger.info("CV {} {}".format(model_id, str(feature.values)))
            sel_cases = mri['case'].loc[np.isfinite(mri.loc[:, feature])]
            X = sfa.loc[sel_cases, :].values
            y = mri.loc[sel_cases, feature].values
            y_pred = cv(model, X, y)
            mse_cv.loc[model_id, feature] =\
                sklearn.metrics.mean_squared_error(y, y_pred)
            mae_cv.loc[model_id, feature] =\
                sklearn.metrics.median_absolute_error(y, y_pred)
            ev_cv.loc[model_id, feature] =\
                sklearn.metrics.explained_variance_score(y, y_pred)

            model.fit(X, y)
            if hasattr(model, 'feature_importances_'):
                feature_importance.loc[model_id, feature, :] =\
